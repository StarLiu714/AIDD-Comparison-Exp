...
PARAMS_JSON_PATH = 'parameters_xg.json'
MODEL_NAME = 'XGBoost'
...
def process_dataset(
    ...
    for values in combinations:
        batch_size, learning_rate, _, _, _, n_tasks, iters = values[:7]
        ...
        for fold, (train_index, valid_index) in enumerate(kf.split(train_valid_data)):
            ...
            if device == torch.device("cuda"):
                ...
                xgb_model = xgb.XGBRegressor(n_estimators=iters, learning_rate=learning_rate, tree_method='gpu_hist')
            else:
                xgb_model = xgb.XGBRegressor(n_estimators=iters, learning_rate=learning_rate)

            dc_model = dc.models.GBDTModel(xgb_model, mode="regression")
