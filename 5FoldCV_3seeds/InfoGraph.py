...
# pip install torch_geometric pytorch_lightning dm-haiku==0.0.10
...
def process_dataset(
    ...
    for values in combinations:
        ...
        for fold, (train_index, valid_index) in enumerate(kf.split(train_valid_data)):
            ...
            edge_dim = max([train_dataset.X[i].num_edge_features for i in range(len(train_dataset))])

            model = InfoGraphStarModel(
                num_feat, edge_dim, n_tasks=n_tasks, mode='regression',
                batch_size=batch_size, embedding_dim=embedding, learning_rate=learning_rate
                )
            if device == "cuda":
